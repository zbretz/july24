...
export default RideDetail = ({ route, isConnected, masterState, navigation, rideHistory }) => {
    ...
    // try {
    //   const res = await fetch('https://your-api.com/api/create-proxy-session', {
    //     method: 'POST',
    ...
    //   if (!res.ok) {
    //     throw new Error('Proxy session could not be created.');
    //   }

    //   const data = await res.json();
    //   const proxyNumber = data.proxyNumber;

    //   const supported = await Linking.canOpenURL(`tel:${proxyNumber}`);
    //   if (supported) {
    //     Linking.openURL(`tel:${proxyNumber}`);
    //   } else {
    //     Alert.alert('Error', 'Device does not support phone calls.');
