                        ...
                        return (
                            <View style={{ borderTopWidth: index == 0 ? 0 : 1, borderTopColor: '#e6e6e6', }} key={item.place_id}>
                                <TouchableHighlight
                                    ...
                                    onPress={() => {
                                        if (type == 'Arrivals') {
                                            setDestination(item.description); setDropoffAddressNotRecognized(false); inputRef.current.blur(); navigation.goBack()// setSearchResults([])
                                        } else {
                                            setPickupLocation(item.description); setPickupAddressNotRecognized(false); inputRef.current.blur(); navigation.goBack()// setSearchResults([])
                                        }
                                    }}>
